...
export default class extends Base {
  ...
  /** 获取棋牌类的推荐游戏 */
  requireRecommends() {
    api.getEgameRecommend({ game_type: 6, page_size: 9 }).then((res: any) => {
      if (res.code === 0) {
        this.recommends = res.data.list || []
        if (this.recommends.length >= 9) {
          this.recommends[8] = {
            game_name: '更多游戏',
            img_url: this.recommends[8].img_url,
            isMore: true
          }
        }
      }
  ...
  toCardGame(item) {
    if (item.isMore) {
      this.$router.push({ path: '/game/recommend', query: { game_type: '6' } })
    } else {
      this.gotoAPI({
